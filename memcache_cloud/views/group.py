...
def groups_index():
    ...
    groups = db_session.query(
        Groups, 
        func.sum(Memcacheds.memory).label('group_total_memory'),
        func.count(Memcacheds.ip).label('memcached_count')
    ).filter(Groups.id==Memcacheds.group_id).group_by(Groups.id).all()
    stats_hit = {}
    stats_useage = {}
    for group in groups :
        group_get = 0
        ...
        stats_useage[group.Groups.id] = {
            "useage": group_useage, "free": group_free, 
            "use_per": group_use_per, "free_per": group_free_per
        }
        if (group_get != 0) :
    ...
    return  render_template('memcache_cloud/groups_index.html', groups = groups, stats_hit = stats_hit, stats_useage = stats_useage)

@mod.route('/group-add')
def group_add():
    return  render_template('memcache_cloud/group_add.html')
