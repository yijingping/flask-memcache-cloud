...
from memcache_cloud.model import Memcacheds
import os
...
def hosts_index():
    ...
    cache_mem = 0
    for host in hosts :
        total_mem = os.popen("ssh -o StrictHostKeyChecking=no " + host.ip + " cat /proc/meminfo|grep MemTotal|awk '{print $2}'").read().strip()
        total_mem = round((float(total_mem) / 1024 / 1024),2)
        free_mem = os.popen("ssh -o StrictHostKeyChecking=no " + host.ip + " cat /proc/meminfo|grep MemFree|awk '{print $2}'").read().strip()
        free_mem = round((float(free_mem) / 1024 / 1024),2)
        cache_mem = os.popen("ssh -o StrictHostKeyChecking=no " + host.ip + " cat /proc/meminfo|grep Cached|head -1|awk '{print $2}'").read().strip()
    ...
    return  render_template('memcache_cloud/host_index.html', 
                            hosts = hosts,
